...
model = AutoModelForCausalLM.from_pretrained(f"facebook/{args.evaluation_model}", torch_dtype=torch.float16, cache_dir=config.data_dir).cuda()
tokenizer = AutoTokenizer.from_pretrained(f"facebook/{args.evaluation_model}", use_fast=False, cache_dir=config.data_dir)
    ...
    with torch.no_grad():
        ...
        for sample in sequences:
            ...
            for generation_index in range(generations.shape[0]):
                ...
                neg_unconditioned_log_likelihoods[generation_index] = average_unconditioned_neg_log_likelihood * (len(generation) - len(prompt))
...
with open(f'{config.data_dir}/{run_name}/{args.generation_model}_generations_{args.evaluation_model}_likelihoods.pkl', 'wb') as outfile:
